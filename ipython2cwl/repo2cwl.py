...
def _store_jn_as_script(
    notebook_path: str,
    git_directory_absolute_path: str,
    bin_absolute_path: str,
    image_id: str,
    ...
    if len(converter._variables) == 0:
        logger.info(
            f"Notebook {notebook_path} does not contains typing annotations. skipping..."
        )
        return None, None
    script_relative_path = os.path.relpath(notebook_path, git_directory_absolute_path)[
        :-6
    ]
    ...
    if len(script_relative_parent_directories) > 1:
        script_absolute_name = os.path.join(
            bin_absolute_path, os.sep.join(script_relative_parent_directories[:-1])
        )
        os.makedirs(script_absolute_name, exist_ok=True)
        script_absolute_name = os.path.join(
            script_absolute_name, os.path.basename(script_relative_path)
        )
    else:
...
def parser_arguments(argv: List[str]):
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Output directory to store the generated cwl files",
        type=existing_path,
        required=True,
    )
...
def repo2cwl(argv: Optional[List[str]] = None) -> int:
    ...
    for tool in cwl_tools:
        base_command_script_name = (
            f'{tool["baseCommand"][len("/app/cwl/bin/"):].replace("/", "_")}.cwl'
        )
...
def _repo2cwl(git_directory_path: Repo) -> Tuple[str, List[Dict]]:
    ...
    for notebook in notebooks_paths:
        cwl_command_line_tool, script_name = _store_jn_as_script(
            notebook, git_directory_path.tree().abspath, bin_path, r2d.output_image_spec
        )
        if cwl_command_line_tool is None or script_name is None:
            continue
        cwl_command_line_tool["baseCommand"] = os.path.join(
            "/app", "cwl", "bin", script_name
        )
